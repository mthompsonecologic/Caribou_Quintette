...
gcinfo(TRUE)
wd <- getwd()
newwd <- paste("/gpfs/scratch/an-rdorador/sat-caribou", "data", "TR/RU/Job_2/", sep = "/")
setwd(newwd)
...
##############Actual Data###############
Caribou_TR_RU<- read.delim("Job_2.txt", sep = "\t")
...
# }
# system.time(
# 	mapply(savingBRB, BRBs_TR_RU, thenames)
# )
# print("#############################################################################")
...
# system.time(
# 	homerange <- mapply(GettingVertices, BRBs_TR_RU, thenames)
# 	)
...
BRBs_TR_RU <- mapply(readRDSData, filenames)
...
system.time(
	vud <- (mapply(GettingVolume, BRBs_TR_RU, thenames))
	)
...
# print("#############################################################################")
# # Save the output.
write.csv(BRB_area, paste("BRB_area/", "/TR_RU_BRB_areas.csv", sep=""), row.names = FALSE)

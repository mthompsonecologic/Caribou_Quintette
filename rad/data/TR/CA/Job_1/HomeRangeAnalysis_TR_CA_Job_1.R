...
gcinfo(TRUE)
wd <- getwd()
newwd <- paste("/gpfs/scratch/an-rdorador/sat-caribou", "data", "TR/CA/Job_1/", sep = "/")
setwd(newwd)
...
# }
# system.time(
# 	mapply(savingBRB, BRBs_TR_CA, thenames)
# )
# print("#############################################################################")
...
# system.time(
# 	homerange <- mapply(GettingVertices, BRBs_TR_CA, thenames)
# 	)
...
BRBs_TR_CA <- mapply(readRDSData, filenames)
...
system.time(
	vud <- (mapply(GettingVolume, BRBs_TR_CA, thenames))
	)
...
# print("#############################################################################")
# # Save the output.
write.csv(BRB_area, paste("BRB_area/", "/TR_CA_BRB_areas.csv", sep=""), row.names = FALSE)

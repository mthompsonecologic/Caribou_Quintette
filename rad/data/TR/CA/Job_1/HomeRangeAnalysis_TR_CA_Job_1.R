wd <- getwd()
newwd <- paste("~/Github/Caribou_Quintette/rad", "data", "TR/CA/Job_1/", sep = "/")
setwd(newwd)
...
##############Actual Data###############
Caribou_TR_CA<- read.delim("Job_1.txt", sep = "\t")
...
years <- unique(Caribou_TR_CA$YearSeas)
...
for(l in 1:length(Caribou_TR_CA_LI)){
  TR_CA_Traj[[l]] <-  as.ltraj(xy = Caribou_TR_CA_LI[[l]][,c("E", "N")], 
                               date =  as.POSIXct(Caribou_TR_CA_LI[[l]]$timestamp,
                                                  format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),
                               id = Caribou_TR_CA_LI[[l]]$AnimalID,
                               typeII = TRUE)
...
names(DLik_TR_CA) <- names(TR_CA_Traj)


DLik_TR_CA_u <- unlist(DLik_TR_CA)
DLik_TR_CA_u <- DLik_TR_CA_u[1:(length(DLik_TR_CA_u)/2)*2]
DLik_TR_CA_u <- as.numeric(as.vector(DLik_TR_CA_u))

Traj_len <- as.numeric(as.vector(lengths(TR_CA_Traj)))


Traj_li <- vector("list", length(DLik_TR_CA_u))
a = 1
b = 1
...
for(y in 1:length(Traj_len)){
  b = 1
  while(b <= Traj_len[y]){
    Traj_li[[a]] <- TR_CA_Traj[[y]][b]
    names(Traj_li[[a]])  <- names(TR_CA_Traj[y])
    b = b + 1
    a = a + 1
  }
}
...
print("##################################BRB Analysis############################################")
system.time({
#   BRBs_TR_CA <- foreach(i = 1:length(Traj_li),
	BRBs_TR_CA <- foreach(i = 1:3,
                        .combine = c,
...
BRBs_TR_CAf <- list.files(here("BRB_UDs"),
                          pattern="\\.Rds$",
                          full.names=TRUE)
...
print("##################################BRB Vertices############################################")
plan(multicore)
BRBs_TR_CA <- lapply(BRBs_TR_CAf, function(x){readRDS(x)})
system.time(BRBs_TR_CAv <- future_lapply(BRBs_TR_CA,
                                         FUN = function(x) {
...
system.time({
  homerange <- foreach(i = 1:length(BRBs_TR_CA),
                       .combine = c,
                       .packages = c("adehabitatHR","here")) %dopar% {
                         saveRDS(getverticeshr.estUD(BRBs_TR_CA[[i]], percent=50),
                                 paste0(here("BRB_UDs","TR","CA"),"/",thenames[i],"_hr.Rds"))
                       }    
})
...
vud <- foreach(i = 1:length(BRBs_TR_CA),
               .combine = c,
               .packages = c("adehabitatHR","here","terra")) %dopar% {
                 vect(getvolumeUD(BRBs_TR_CA[[i]]),
                      paste0(here("BRB_UDs","TR","CA"),"/",
                             thenames[i],"_vUD.shp"))    
               }
...
system.time(for(i in 1:length(thenames)){
  ...
  BRB_area[i,c(1:4)] <- rbind(data.frame(id = name_burst[[i]],
                                         year = substr(names(TR_CA_Traj[i]),7,11),
                                         area = homerangedf[,2],
                                         nb.reloc = nrow(TR_CA_Traj[[i]][[1]])))
})
print("#############################################################################")
# Save the output.
write.csv(BRB_area, paste0(here("BRB_UDs"), "/TR_CA_BRB_areas.csv", sep=""), row.names = FALSE)

...
gcinfo(TRUE)
wd <- getwd()
newwd <- paste("/gpfs/scratch/an-rdorador/sat-caribou", "data", "TR/SU/Job_0/", sep = "/")
setwd(newwd)
...
# }
# system.time(
# 	mapply(savingBRB, BRBs_TR_SU, thenames)
# )
# print("#############################################################################")
...
# system.time(
# 	homerange <- mapply(GettingVertices, BRBs_TR_SU, thenames)
# 	)
...
BRBs_TR_SU <- mapply(readRDSData, filenames)
...
system.time(
	vud <- (mapply(GettingVolume, BRBs_TR_SU, thenames))
	)
...
# print("#############################################################################")
# # Save the output.
write.csv(BRB_area, paste("BRB_area/", "/TR_SU_BRB_areas.csv", sep=""), row.names = FALSE)

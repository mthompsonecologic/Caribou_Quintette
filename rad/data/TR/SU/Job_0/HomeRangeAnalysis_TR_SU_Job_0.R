wd <- getwd()
newwd <- paste("~/Github/Caribou_Quintette/rad", "data", "TR/SU/Job_0/", sep = "/")
setwd(newwd)
...
##############Actual Data###############
Caribou_TR_SU<- read.delim("Job_0.txt", sep = "\t")
...
years <- unique(Caribou_TR_SU$YearSeas)
...
for(l in 1:length(Caribou_TR_SU_LI)){
  TR_SU_Traj[[l]] <-  as.ltraj(xy = Caribou_TR_SU_LI[[l]][,c("E", "N")], 
                               date =  as.POSIXct(Caribou_TR_SU_LI[[l]]$timestamp,
                                                  format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),
                               id = Caribou_TR_SU_LI[[l]]$AnimalID,
                               typeII = TRUE)
...
names(DLik_TR_SU) <- names(TR_SU_Traj)


DLik_TR_SU_u <- unlist(DLik_TR_SU)
DLik_TR_SU_u <- DLik_TR_SU_u[1:(length(DLik_TR_SU_u)/2)*2]
DLik_TR_SU_u <- as.numeric(as.vector(DLik_TR_SU_u))

Traj_len <- as.numeric(as.vector(lengths(TR_SU_Traj)))


Traj_li <- vector("list", length(DLik_TR_SU_u))
a = 1
b = 1
...
for(y in 1:length(Traj_len)){
  b = 1
  while(b <= Traj_len[y]){
    Traj_li[[a]] <- TR_SU_Traj[[y]][b]
    names(Traj_li[[a]])  <- names(TR_SU_Traj[y])
    b = b + 1
    a = a + 1
  }
}
...
print("##################################BRB Analysis############################################")
system.time({
#   BRBs_TR_SU <- foreach(i = 1:length(Traj_li),
	BRBs_TR_SU <- foreach(i = 1:3,
                        .combine = c,
...
BRBs_TR_SUf <- list.files(here("BRB_UDs"),
                          pattern="\\.Rds$",
                          full.names=TRUE)
...
print("##################################BRB Vertices############################################")
plan(multicore)
BRBs_TR_SU <- lapply(BRBs_TR_SUf, function(x){readRDS(x)})
system.time(BRBs_TR_SUv <- future_lapply(BRBs_TR_SU,
                                         FUN = function(x) {
...
system.time({
  homerange <- foreach(i = 1:length(BRBs_TR_SU),
                       .combine = c,
                       .packages = c("adehabitatHR","here")) %dopar% {
                         saveRDS(getverticeshr.estUD(BRBs_TR_SU[[i]], percent=50),
                                 paste0(here("BRB_UDs","TR","SU"),"/",thenames[i],"_hr.Rds"))
                       }    
})
...
vud <- foreach(i = 1:length(BRBs_TR_SU),
               .combine = c,
               .packages = c("adehabitatHR","here","terra")) %dopar% {
                 vect(getvolumeUD(BRBs_TR_SU[[i]]),
                      paste0(here("BRB_UDs","TR","SU"),"/",
                             thenames[i],"_vUD.shp"))    
               }

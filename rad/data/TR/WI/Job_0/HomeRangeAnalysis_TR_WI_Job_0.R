wd <- getwd()
newwd <- paste("~/Github/Caribou_Quintette/rad", "data", "TR/WI/Job_0/", sep = "/")
setwd(newwd)
...
##############Actual Data###############
Caribou_TR_WI<- read.delim("Job_0.txt", sep = "\t")
...
years <- unique(Caribou_TR_WI$YearSeas)
...
names(DLik_TR_WI) <- names(TR_WI_Traj)


DLik_TR_WI_u <- unlist(DLik_TR_WI)
DLik_TR_WI_u <- DLik_TR_WI_u[1:(length(DLik_TR_WI_u)/2)*2]
DLik_TR_WI_u <- as.numeric(as.vector(DLik_TR_WI_u))

Traj_len <- as.numeric(as.vector(lengths(TR_WI_Traj)))


Traj_li <- vector("list", length(DLik_TR_WI_u))
a = 1
b = 1
...
for(y in 1:length(Traj_len)){
  b = 1
  while(b <= Traj_len[y]){
    Traj_li[[a]] <- TR_WI_Traj[[y]][b]
    names(Traj_li[[a]])  <- names(TR_WI_Traj[y])
    b = b + 1
    a = a + 1
  }
}
...
print("##################################BRB Analysis############################################")
system.time({
#   BRBs_TR_WI <- foreach(i = 1:length(Traj_li),
	BRBs_TR_WI <- foreach(i = 1:3,
                        .combine = c,
...
BRBs_TR_WIf <- list.files(here("BRB_UDs"),
                          pattern="\\.Rds$",
                          full.names=TRUE)
...
print("##################################BRB Vertices############################################")
plan(multicore)
BRBs_TR_WI <- lapply(BRBs_TR_WIf, function(x){readRDS(x)})
system.time(BRBs_TR_WIv <- future_lapply(BRBs_TR_WI,
                                         FUN = function(x) {
...
system.time({
  homerange <- foreach(i = 1:length(BRBs_TR_WI),
                       .combine = c,
                       .packages = c("adehabitatHR","here")) %dopar% {
                         saveRDS(getverticeshr.estUD(BRBs_TR_WI[[i]], percent=50),
                                 paste0(here("BRB_UDs","TR","WI"),"/",thenames[i],"_hr.Rds"))
                       }    
})
...
vud <- foreach(i = 1:length(BRBs_TR_WI),
               .combine = c,
               .packages = c("adehabitatHR","here","terra")) %dopar% {
                 vect(getvolumeUD(BRBs_TR_WI[[i]]),
                      paste0(here("BRB_UDs","TR","WI"),"/",
                             thenames[i],"_vUD.shp"))    
               }
...
system.time(for(i in 1:length(thenames)){
  ...
  BRB_area[i,c(1:4)] <- rbind(data.frame(id = name_burst[[i]],
                                         year = substr(names(TR_WI_Traj[i]),7,11),
                                         area = homerangedf[,2],
                                         nb.reloc = nrow(TR_WI_Traj[[i]][[1]])))
})
print("#############################################################################")
# Save the output.

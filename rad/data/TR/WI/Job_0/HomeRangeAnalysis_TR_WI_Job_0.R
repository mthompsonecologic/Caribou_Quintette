...
gcinfo(TRUE)
wd <- getwd()
newwd <- paste("/gpfs/scratch/an-rdorador/sat-caribou", "data", "TR/WI/Job_0/", sep = "/")
setwd(newwd)
...
BRBs_TR_WI <- mapply(readRDSData, filenames)
...
# print("#############################################################################")
# # Save the output.
write.csv(BRB_area, paste("BRB_area/", "/TR_WI_BRB_areas.csv", sep=""), row.names = FALSE)

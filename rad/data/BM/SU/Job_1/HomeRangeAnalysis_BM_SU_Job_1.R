...
gcinfo(TRUE)
wd <- getwd()
newwd <- paste("/gpfs/scratch/an-rdorador/sat-caribou", "data", "BM/SU/Job_1/", sep = "/")
setwd(newwd)
...
# }
# system.time(
# 	mapply(savingBRB, BRBs_BM_SU, thenames)
# )
# print("#############################################################################")
...
# system.time(
# 	homerange <- mapply(GettingVertices, BRBs_BM_SU, thenames)
# 	)
...
BRBs_BM_SU <- mapply(readRDSData, filenames)
...
system.time(
	vud <- (mapply(GettingVolume, BRBs_BM_SU, thenames))
	)
...
# print("#############################################################################")
# # Save the output.
write.csv(BRB_area, paste("BRB_area/", "/BM_SU_BRB_areas.csv", sep=""), row.names = FALSE)

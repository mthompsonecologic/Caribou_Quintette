wd <- getwd()
newwd <- paste("~/Github/Caribou_Quintette/rad", "data", "BM/SU/Job_4/", sep = "/")
setwd(newwd)
...
##############Actual Data###############
Caribou_BM_SU<- read.delim("Job_4.txt", sep = "\t")
...
years <- unique(Caribou_BM_SU$YearSeas)
...
names(DLik_BM_SU) <- names(BM_SU_Traj)


DLik_BM_SU_u <- unlist(DLik_BM_SU)
DLik_BM_SU_u <- DLik_BM_SU_u[1:(length(DLik_BM_SU_u)/2)*2]
DLik_BM_SU_u <- as.numeric(as.vector(DLik_BM_SU_u))

Traj_len <- as.numeric(as.vector(lengths(BM_SU_Traj)))


Traj_li <- vector("list", length(DLik_BM_SU_u))
a = 1
b = 1
...
for(y in 1:length(Traj_len)){
  b = 1
  while(b <= Traj_len[y]){
    Traj_li[[a]] <- BM_SU_Traj[[y]][b]
    names(Traj_li[[a]])  <- names(BM_SU_Traj[y])
    b = b + 1
    a = a + 1
  }
}
...
print("##################################BRB Analysis############################################")
system.time({
#   BRBs_BM_SU <- foreach(i = 1:length(Traj_li),
	BRBs_BM_SU <- foreach(i = 1:3,
                        .combine = c,
...
BRBs_BM_SUf <- list.files(here("BRB_UDs"),
                          pattern="\\.Rds$",
                          full.names=TRUE)
...
print("##################################BRB Vertices############################################")
plan(multicore)
BRBs_BM_SU <- lapply(BRBs_BM_SUf, function(x){readRDS(x)})
system.time(BRBs_BM_SUv <- future_lapply(BRBs_BM_SU,
                                         FUN = function(x) {
...
system.time({
  homerange <- foreach(i = 1:length(BRBs_BM_SU),
                       .combine = c,
                       .packages = c("adehabitatHR","here")) %dopar% {
                         saveRDS(getverticeshr.estUD(BRBs_BM_SU[[i]], percent=50),
                                 paste0(here("BRB_UDs","BM","SU"),"/",thenames[i],"_hr.Rds"))
                       }    
})
...
vud <- foreach(i = 1:length(BRBs_BM_SU),
               .combine = c,
               .packages = c("adehabitatHR","here","terra")) %dopar% {
                 vect(getvolumeUD(BRBs_BM_SU[[i]]),
                      paste0(here("BRB_UDs","BM","SU"),"/",
                             thenames[i],"_vUD.shp"))    
               }
...
system.time(for(i in 1:length(thenames)){
  ...
  BRB_area[i,c(1:4)] <- rbind(data.frame(id = name_burst[[i]],
                                         year = substr(names(BM_SU_Traj[i]),7,11),
                                         area = homerangedf[,2],
                                         nb.reloc = nrow(BM_SU_Traj[[i]][[1]])))
})
print("#############################################################################")
# Save the output.

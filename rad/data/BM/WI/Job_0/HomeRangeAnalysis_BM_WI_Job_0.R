...
gcinfo(TRUE)
wd <- getwd()
newwd <- paste("/gpfs/scratch/an-rdorador/sat-caribou", "data", "BM/WI/Job_0/", sep = "/")
setwd(newwd)
...
##############Actual Data###############
Caribou_BM_WI<- read.delim("Job_0.txt", sep = "\t")
...
BRBs_BM_WI <- mapply(readRDSData, filenames)
...
# print("#############################################################################")
# # Save the output.
write.csv(BRB_area, paste("BRB_area/", "/BM_WI_BRB_areas.csv", sep=""), row.names = FALSE)

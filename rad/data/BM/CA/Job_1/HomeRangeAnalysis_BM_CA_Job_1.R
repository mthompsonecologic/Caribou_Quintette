...
gcinfo(TRUE)
wd <- getwd()
newwd <- paste("/gpfs/scratch/an-rdorador/sat-caribou", "data", "BM/CA/Job_1/", sep = "/")
setwd(newwd)
...
##############Actual Data###############
Caribou_BM_CA<- read.delim("Job_1.txt", sep = "\t")
...
BRBs_BM_CA <- mapply(readRDSData, filenames)
...
# print("#############################################################################")
# # Save the output.
write.csv(BRB_area, paste("BRB_area/", "/BM_CA_BRB_areas.csv", sep=""), row.names = FALSE)

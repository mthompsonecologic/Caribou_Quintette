...
gcinfo(TRUE)
wd <- getwd()
newwd <- paste("/gpfs/scratch/an-rdorador/sat-caribou", "data", "BM/RU/Job_3/", sep = "/")
setwd(newwd)
...
##############Actual Data###############
Caribou_BM_RU<- read.delim("Job_3.txt", sep = "\t")
...
system.time(
	vud <- (mapply(GettingVolume, BRBs_BM_RU, thenames))
	)

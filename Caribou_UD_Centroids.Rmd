...
for(n in 1:length(BM_sUDs)){
  for(y in 1:length(BM_sUDs[[n]])){
    if(y < length(BM_sUDs[[n]])-1){
      ...
      x1$ID <-1
      x2$ID <-2
      x1 <- st_as_sf(x1, crs = 4269)
      x2 <- st_as_sf(x2, crs = 4269)
      x1 <- x1[,c(1,4,3)]
      x2 <- x2[,c(1,4,3)]

      BM_chSum[[lc]] <- assign(paste0(BM_sUDsn[n],"_",substr(names(BM_sUDs[[n]][y]), 8, 11)),stampr::stamp(x1, x2, direction = FALSE, distance = FALSE, shape=FALSE))
...
for(n in 1:length(TR_sUDs)){
  for(y in 1:length(TR_sUDs[[n]])){
    if(y < length(TR_sUDs[[n]])-1){
      ...
      x2$ID <- as.integer(2)
      x1 <- st_as_sf(x1, crs = 4269)
      x1 <- st_cast(x1, "MULTIPOLYGON") %>% st_cast("POLYGON")
      x2 <- st_as_sf(x2, crs = 4269)
      x2 <- st_cast(x2, "MULTIPOLYGON") %>% st_cast("POLYGON")
      x1 <- x1[,c(1,4,3)]
      x2 <- x2[,c(1,4,3)]
      TR_chSum[[lc]] <- assign(paste0(TR_sUDsn[n],"_",substr(names(TR_sUDs[[n]][y]), 8, 11)),stampr::stamp(x1, x2,direction = TRUE, distance = TRUE))
...
QU_All.shp <- project(QU_All.shp, crs(BMWI[[1]]))
...
for(i in 1:length(TR_sUDsn)){
  assign(paste0("WRbase_",sUDsn[i]),tm_shape(QU_All_SA.shp, bbox = bbox_new) + tm_fill("CH_TYPE_FI", palette=c("wheat3","lightblue1")) + tm_borders() + tm_layout(frame = FALSE, main.title = paste0("Centroids of 50% UDs Tracked by Season: ", sUDsn[i]), legend.show = FALSE)  + tm_scale_bar(position = c("right","top")))
}
...
tcrop <- rast(nrows=5493, ncols=6908, nlyrs=1, xmin=565000, xmax=650000, 
...
centrack_TRSU <- Trackbase_TRSU + cenli_TRSU_1 + cenli_TRSU_2 + cenli_TRSU_3 + cenli_TRSU_4 + cenli_TRSU_5 + cenli_TRSU_6 + cenli_TRSU_7 + cenli_TRSU_8 + cenli_TRSU_9 + cenli_TRSU_10 + cenli_TRSU_11 + cenli_TRSU_12 + cenli_TRSU_13 + cenli_TRSU_14 + cenli_TRSU_15 + cen_TRSU1 + cen_TRSU2 + cen_TRSU3 + cen_TRSU4 + cen_TRSU5 + cen_TRSU6 + cen_TRSU7 + cen_TRSU8 + cen_TRSU9 + cen_TRSU10 + cen_TRSU11 + cen_TRSU12 + cen_TRSU13 + cen_TRSU14 + cen_TRSU15
...
### Enlarged areas
```{r , echo=FALSE}
centrack_Agg_WI <- WRbase_WI + cenli_BMWI_1 + cenli_BMWI_2 + cenli_BMWI_3 + cenli_BMWI_4 + cenli_BMWI_5 + cenli_BMWI_6 + cenli_BMWI_7 + cenli_BMWI_8 + cenli_BMWI_9 + cenli_BMWI_10 + cenli_BMWI_11 + cenli_BMWI_12 + cenli_BMWI_13 + cenli_BMWI_14 + cenli_BMWI_15 + cenli_BMWI_16 + cenli_BMWI_17+ cenli_BMWI_18 + cen_BMWI1 + cen_BMWI2 + cen_BMWI3 + cen_BMWI4 + cen_BMWI5 + cen_BMWI6 + cen_BMWI7 + cen_BMWI8 + cen_BMWI9 + cen_BMWI10 + cen_BMWI11 + cen_BMWI12 + cen_BMWI13 + cen_BMWI14 + cen_BMWI15 + cen_BMWI16 + cen_BMWI17 + cen_BMWI18 + cen_BMWI19 + cenli_TRWI_1 + cenli_TRWI_2 + cenli_TRWI_3 + cenli_TRWI_4 + cenli_TRWI_5 + cenli_TRWI_6 + cenli_TRWI_7 + cenli_TRWI_8 + cenli_TRWI_9 + cenli_TRWI_10 + cenli_TRWI_11 + cenli_TRWI_12 + cenli_TRWI_13 + cenli_TRWI_14 + cenli_TRWI_15 + cenli_TRWI_16 + cen_TRWI1 + cen_TRWI2 + cen_TRWI3 + cen_TRWI4 + cen_TRWI5 + cen_TRWI6 + cen_TRWI7 + cen_TRWI8 + cen_TRWI9 + cen_TRWI10 + cen_TRWI11 + cen_TRWI12 + cen_TRWI13 + cen_TRWI14 + cen_TRWI15 + cen_TRWI16 + cen_TRWI17
...
centrack_Agg_SU <- WRbase_SU + cenli_BMSU_1 + cenli_BMSU_2 + cenli_BMSU_3 + cenli_BMSU_4 + cenli_BMSU_5 + cenli_BMSU_6 + cenli_BMSU_7 + cenli_BMSU_8 + cenli_BMSU_9 + cenli_BMSU_10 + cenli_BMSU_11 + cenli_BMSU_12 + cenli_BMSU_13 + cenli_BMSU_14 + cenli_BMSU_15 + cenli_BMSU_16 + cen_BMSU1 + cen_BMSU2 + cen_BMSU3 + cen_BMSU4 + cen_BMSU5 + cen_BMSU6 + cen_BMSU7 + cen_BMSU8 + cen_BMSU9 + cen_BMSU10 + cen_BMSU11 + cen_BMSU12 + cen_BMSU13 + cen_BMSU14 + cen_BMSU15 + cen_BMSU16 + cen_BMSU17 + cen_BMSU18 + cenli_TRSU_1 + cenli_TRSU_2 + cenli_TRSU_3 + cenli_TRSU_4 + cenli_TRSU_5 + cenli_TRSU_6 + cenli_TRSU_7 + cenli_TRSU_8 + cenli_TRSU_9 + cenli_TRSU_10 + cenli_TRSU_11 + cenli_TRSU_12 + cenli_TRSU_13 + cenli_TRSU_14 + cenli_TRSU_15 + cen_TRSU1 + cen_TRSU2 + cen_TRSU3 + cen_TRSU4 + cen_TRSU5 + cen_TRSU6 + cen_TRSU7 + cen_TRSU8 + cen_TRSU9 + cen_TRSU10 + cen_TRSU11 + cen_TRSU12 + cen_TRSU13 + cen_TRSU14 + cen_TRSU15

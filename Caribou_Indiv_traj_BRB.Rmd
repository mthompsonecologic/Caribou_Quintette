...
#create the cluster
# One local machine, reduce the number of cores to save mem.
...
system.time(for(i in 1:length(thenames)){
	vect(BRBs_BM_WIv[i],paste0(here("BRB_UDs","BM","WI"),"/",thenames[i],"_hr.Rds"))
  homerangedf <- as.data.frame(BRBs_BM_WIv[i])
  BRB_area[i,c(1:4)] <- rbind(data.frame(id = name_burst[[i]], year = substr(names(BM_WI_Traj[i]),7,11), area = homerangedf[,2], nb.reloc = nrow(BM_WI_Traj[[i]][[1]])))
     })
...
plan(multicore)
BRBs_BM_WI <- lapply(BRBs_BM_WIf, function(x){readRDS(x)})
system.time(BRBs_BM_WIv <- future_lapply(BRBs_BM_CA, FUN = function(x) {getverticeshr.estUD(x)}))
system.time(for(i in 1:length(thenames)){
	vect(BRBs_BM_CAv[i],paste0(here("BRB_UDs"),"/",thenames[i],"_hr.Rds"))
     })

...
########################################
### ACENET Stop here - no ggplot needed.
...
vud <- foreach(i = 1:length(BRBs_BM_WI),
                      .combine = c,
                      .packages = c("adehabitatHR","adehabitatLT")) %dopar% {
                      saveRDS(getvolumeUD(BRBs_BM_WI[[i]]),
                              paste0(here("BRB_UDs","/",thenames,"_vud.Rds")))
                        }    
	})
...
homerange <- foreach(i = 1:length(BRBs_BM_WI),
                      ...
                      saveRDS(getverticeshr.estUD(BRBs_BM_WI[[i]]),
                              paste0(here("BRB_UDs","/",thenames,"_hr.Rds")))
                        }    
	})
# Stop the parallel backend
stopCluster(my.cluster)
...
my.cluster <- parallel::makeCluster(
  n.cores, 
  type = "FORK"  ## fork gives Bus error!!
  )
doParallel::registerDoParallel(cl = my.cluster)

foreach(i = 1:length(thenames),
        .combine = c) %dopar% {
          saveRDS(BRBs_BM_WI[[i]],
                  paste0(here("BRB_UDs"),"/",thenames[i],".Rds"))}

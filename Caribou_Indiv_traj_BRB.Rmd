...
# RAD --- run up to here for the acenet interactive version --- 
...
# RAD - RN ENTIRE CHNK BELOW FOR AC$NET INTERACTIVE (USE SALLOC)
```{r echo=FALSE}
...
# ----RAD Convert above into a for loop ----- 
...
BRB home range analysis - Test Example -- SKIP BELOW CHNK -- RAD
...
stopCluster(my.cluster)
# RAD __ ABOVE WORKS -- BELOW VARIOUS ATTEMPTS ___ goal is to get BRBs_BW_WI and save the it as an intermediary -- R max workerts is 125 (in practice less) -- SLURM: ntasks = nodes; cpus seems to be number of cores

# clusterExport(cl = my.cluster, BRBs_BM_WI)
BRBs_BM_WIv <- parLapply(cl = my.cluster, BRBs_BM_WI, function (x) {vect(x[1])})
BRBs_BM_WIv <- parLapply(cl = my.cluster, brb, vect)
stopCluster(my.cluster)
...
doParallel::registerDoParallel(cl = my.cluster)
clusterExport(cl = my.cluster, BRBs_BM_WI)
parLapply(cl=my.cluster, X = BRBs_BM_WIv, FUN = WV)
stopCluster(my.cluster)

###############################################################
################################################################
## Save as shapefiles:
# I struggled here to find a way to save with the names in the apply
# functions. A work around was created by using 'thenames', otherwise
# the files get saved as 'dens.shp'. I think the challenge is a property
# of the adhabitatHR class that would have to be researched in its repo.

BRBs_BM_WIv <- mclapply(BRBs_BM_WI,function (x) {vect(x)}, mc.cores = n.cores)

names(BRBs_BM_WI) <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
...
#sapply(BRBs_BM_WIv, FUN=WV, simplify=FALSE, USE.NAMES = TRUE)
...
BRBs_BM_WIv <- foreach(i = 1:length(BRBs_BM_WI),
                      .combine = c,
...
names(BRBs_BM_WIv) <- names(BM_WI_Traj)
################################################################
################################################################
...
WVvol <- function (x) {writeVector(x,
                            paste0(here("BRB_UDs"), "/",
                            thenames,"vud.shp"),
                            overwrite=TRUE)}

#sapply(vud, FUN=WVvol, simplify=FALSE, USE.NAMES = TRUE)
mcmapply(vud, FUN=WVvol, simplify=FALSE, USE.NAMES = TRUE, mc.cores = n.cores)
...
# Save
WVver <- function (x) {writeVector(x,
                            paste0(here("BRB_UDs"), "/",
                            thenames,"ver.shp"),
                            overwrite=TRUE)}

#sapply(homerange, FUN=WVver, simplify=FALSE, USE.NAMES = TRUE)
mcmapply(homerange, FUN=WVver, simplify=FALSE, USE.NAMES = TRUE, mc.cores = n.cores)
...
for(i in 1:length(DLik_BM_CA_u)){
  for(y in 1:length(Traj_len)){
    if(a <= Traj_len[y]){
      Traj_li[[i]] <- BM_CA_Traj[[y]][a]
      a = a+1
    }else{a = 1
    next()}
  }
...
names(BRBs_BM_CA) <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
thenames <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
 
BRBs_BM_CAv <- lapply(BRBs_BM_CA, function (x) vect(x))
names(BRBs_BM_CAv) <- names(BM_CA_Traj)
...
sapply(BRBs_BM_CAv, FUN=WV, simplify=FALSE, USE.NAMES = TRUE)
...
homerange <- foreach(i = 1:length(BRBs_BM_CA),
                      .combine = c,
                      .packages = c("adehabitatHR")) %dopar% {
                      getverticeshr.estUD(BRBs_BM_CA[[i]])    
                      }
...
for(i in 1:length(DLik_BM_SU_u)){
  for(y in 1:length(Traj_len)){
    if(a <= Traj_len[y]){
      Traj_li[[i]] <- BM_SU_Traj[[y]][a]
      a = a+1
    }else{a = 1
    next()}
  }
...
names(BRBs_BM_SU) <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
thenames <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
 
BRBs_BM_SUv <- lapply(BRBs_BM_SU, function (x) vect(x))
names(BRBs_BM_SUv) <- names(BM_SU_Traj)
...
sapply(BRBs_BM_SUv, FUN=WV, simplify=FALSE, USE.NAMES = TRUE)
...
homerange <- foreach(i = 1:length(BRBs_BM_SU),
                      .combine = c,
                      .packages = c("adehabitatHR")) %dopar% {
                      getverticeshr.estUD(BRBs_BM_SU[[i]])    
                      }
...
for(i in 1:length(DLik_BM_RU_u)){
  for(y in 1:length(Traj_len)){
    if(a <= Traj_len[y]){
      Traj_li[[i]] <- BM_RU_Traj[[y]][a]
      a = a+1
    }else{a = 1
    next()}
  }
...
names(BRBs_BM_RU) <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
thenames <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
 
BRBs_BM_RUv <- lapply(BRBs_BM_RU, function (x) vect(x))
names(BRBs_BM_RUv) <- names(BM_RU_Traj)
...
sapply(BRBs_BM_RUv, FUN=WV, simplify=FALSE, USE.NAMES = TRUE)
...
homerange <- foreach(i = 1:length(BRBs_BM_RU),
                      .combine = c,
                      .packages = c("adehabitatHR")) %dopar% {
                      getverticeshr.estUD(BRBs_BM_RU[[i]])    
                      }
...
This chunk repeats what was performed for Bullmoose BRBs.
...
# Note to understand resultant data structure:
#sum(lengths(DLik_TR_WI))
#[1] 150
#sum(Traj_len)
#[1] 150

## The BRBs_TR_WI need to be named. The names are contained here:
...
#ntasks = length(Traj_li)
#ntasks_pc = round(ntasks/n.cores,down)
...
## Save as shapefiles:
# I struggled here to find a way to save with the names in the apply
# functions. This is a work around using 'thenames', otherwise
# the files get saved as 'dens.shp'. It must be a property of the
# adhabitatHR class.

names(BRBs_TR_WI) <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
thenames <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
 
BRBs_TR_WIv <- lapply(BRBs_TR_WI, function (x) vect(x))
names(BRBs_TR_WIv) <- names(TR_WI_Traj)
...
#sapply(BRBs_TR_WIv, FUN=WV, simplify=FALSE, USE.NAMES = TRUE)
#See: https://bookdown.org/rdpeng/rprogdatascience/parallel-computation.html
mcmapply(BRBs_TR_WIv, FUN=WV, simplify=FALSE, USE.NAMES = TRUE, mc.cores = n.cores)
...
homerange <- foreach(i = 1:length(BRBs_TR_WI),
                      .combine = c,
                      .packages = c("adehabitatHR")) %dopar% {
                      getverticeshr.estUD(BRBs_TR_WI[[i]])    
                      }
...
for(i in 1:length(DLik_TR_CA_u)){
  for(y in 1:length(Traj_len)){
    if(a <= Traj_len[y]){
      Traj_li[[i]] <- TR_CA_Traj[[y]][a]
      a = a+1
    }else{a = 1
    next()}
  }
...
names(BRBs_TR_CA) <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
thenames <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
 
BRBs_TR_CAv <- lapply(BRBs_TR_CA, function (x) vect(x))
names(BRBs_TR_CAv) <- names(TR_CA_Traj)
...
sapply(BRBs_TR_CAv, FUN=WV, simplify=FALSE, USE.NAMES = TRUE)
...
homerange <- foreach(i = 1:length(BRBs_TR_CA),
                      .combine = c,
                      .packages = c("adehabitatHR")) %dopar% {
                      getverticeshr.estUD(BRBs_TR_CA[[i]])    
                      }
...
for(i in 1:length(DLik_TR_SU_u)){
  for(y in 1:length(Traj_len)){
    if(a <= Traj_len[y]){
      Traj_li[[i]] <- TR_SU_Traj[[y]][a]
      a = a+1
    }else{a = 1
    next()}
  }
...
names(BRBs_TR_SU) <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
thenames <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
 
BRBs_TR_SUv <- lapply(BRBs_TR_SU, function (x) vect(x))
names(BRBs_TR_SUv) <- names(TR_SU_Traj)
...
sapply(BRBs_TR_SUv, FUN=WV, simplify=FALSE, USE.NAMES = TRUE)
...
homerange <- foreach(i = 1:length(BRBs_TR_SU),
                      .combine = c,
                      .packages = c("adehabitatHR")) %dopar% {
                      getverticeshr.estUD(BRBs_TR_SU[[i]])    
                      }
...
for(i in 1:length(DLik_TR_RU_u)){
  for(y in 1:length(Traj_len)){
    if(a <= Traj_len[y]){
      Traj_li[[i]] <- TR_RU_Traj[[y]][a]
      a = a+1
    }else{a = 1
    next()}
  }
...
names(BRBs_TR_RU) <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
thenames <- unlist(lapply(Traj_li, function (x) paste0(names(x),"_",id(x))))
 
BRBs_TR_RUv <- lapply(BRBs_TR_RU, function (x) vect(x))
names(BRBs_TR_RUv) <- names(TR_RU_Traj)
...
sapply(BRBs_TR_RUv, FUN=WV, simplify=FALSE, USE.NAMES = TRUE)
...
homerange <- foreach(i = 1:length(BRBs_TR_RU),
                      .combine = c,
                      .packages = c("adehabitatHR")) %dopar% {
                      getverticeshr.estUD(BRBs_TR_RU[[i]])    
                      }

...
This RMarkdown script is designed to:
...
Caribou_Quintette <- read.csv(paste0(here("Input Data"), "/Caribou_noclip.csv"))
## Adjusting order to fit with the original data that was used to build this script after data updates provided by SM in Aug 2022.
#Caribou_Quintette$year <- format.Date(Caribou_Quintette$timestamp, format = "%Y")

#Caribou_Quintette <- Caribou_Quintette[,c(1:3,5,7:8,4,10,9)]
names(Caribou_Quintette)[names(Caribou_Quintette) == "Season2"] <- "season"

## There was a mistake with Calvin and Calving, fixed:
Caribou_Quintette[which(Caribou_Quintette$season == "Calvin"),4] <- "Calving"

assign_seasonYR <- function(d) {
  ifelse(format(as.Date(d, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),"%m") == "11" & format(as.Date(d, format =  "%Y-%m-%d %H:%M:%S", tz = "GMT"),"%d") >= "1", as.numeric(format(as.Date(d, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),"%Y"))+1,
  ifelse(format(as.Date(d, format =  "%Y-%m-%d %H:%M:%S", tz = "GMT"),"%m") == "12" & format(as.Date(d, format =  "%Y-%m-%d %H:%M:%S", tz = "GMT"),"%d") <= "31", as.numeric(format(as.Date(d, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),"%Y")), as.numeric(format(as.Date(d, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),"%Y"))))
}

## If the winter season is Nov 1 to Dec 31, then it is the year+1 winter season.
Caribou_Quintette$YearSeas = sapply(Caribou_Quintette$timestamp, assign_seasonYR)

Caribou_Quintette$WIdate <- paste0(Caribou_Quintette$YearSeas,substr(as.character(Caribou_Quintette$timestamp),5,10)," ",substr(as.character(Caribou_Quintette$timestamp),12,19))
Caribou_Quintette$WIdate <- as.POSIXct(Caribou_Quintette$WIdate, format = "%Y-%m-%d %H:%M:%S", tz = "GMT")

Caribou_Quintette$SA[Caribou_Quintette$SA == "BM"] <- "CCBS"
Caribou_Quintette$SA[Caribou_Quintette$SA == "TR"] <- "BKRQ"

Caribou_BM <- Caribou_Quintette[which(Caribou_Quintette$SA == "CCBS"),]
Caribou_TR <- Caribou_Quintette[which(Caribou_Quintette$SA == "BKRQ"),]
...
## use Caribou_BM$timestamp for actual date, or Caribou_BM$WIdate to set early winter with year+1
CaribouBM.ltraj <- as.ltraj(xy = Caribou_BM[,c("E", "N")], 
                       date =  as.POSIXct(Caribou_BM$WIdate, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),
...
#BM_path.plot # call plot

## Trend Roman
## use Caribou_TR$timestamp for actual date, or Caribou_TR$WIdate to set early winter with year+1

CaribouTR.ltraj <- as.ltraj(xy = Caribou_TR[,c("E", "N")], 
                       date =  as.POSIXct(Caribou_TR$WIdate, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),
...
Caribou_BM_WI <- Caribou_BM[which(Caribou_BM$season == "Winter"),]
Caribou_BM_CA <- Caribou_BM[which(Caribou_BM$season == "Calving"),]
Caribou_BM_SU <- Caribou_BM[which(Caribou_BM$season == "Summer"),]
Caribou_BM_RU <- Caribou_BM[which(Caribou_BM$season == "Rut"),]
Caribou_TR_WI <- Caribou_TR[which(Caribou_TR$season == "Winter"),]
Caribou_TR_CA <- Caribou_TR[which(Caribou_TR$season == "Calving"),]
Caribou_TR_SU <- Caribou_TR[which(Caribou_TR$season == "Summer"),]
Caribou_TR_RU <- Caribou_TR[which(Caribou_TR$season == "Rut"),]

years <- unique(Caribou_Quintette$YearSeas)
...
## This was a check that the summer dates stayed the same after the winter seasonal correction was made (e.g., Nov-Dec = %Y+1 to fit in Jan-Apr %Y) - test = TRUE, so it is good.
#Caribou_BM_SU_LI <- lapply(Caribou_BM_SU_LI, function(x) {
#      x[,5] <- as.POSIXct(as.Date(x[,5]))
#      x })
#
#test <- lapply(Caribou_BM_SU_LI, function(x) {
#      x <- x[,5] == x[,5]
#      x })
...
for(l in 1:length(Caribou_BM_CA_LI)){
  BM_CA_Traj[[l]] <-  as.ltraj(xy = Caribou_BM_CA_LI[[l]][,c("E", "N")], 
                          date =  as.POSIXct(Caribou_BM_CA_LI[[l]]$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),
                          id = Caribou_BM_CA_LI[[l]]$AnimalID, typeII = TRUE)
...
for(l in 1:length(Caribou_BM_SU_LI)){
  BM_SU_Traj[[l]] <-  as.ltraj(xy = Caribou_BM_SU_LI[[l]][,c("E", "N")], 
                          date =  as.POSIXct(Caribou_BM_SU_LI[[l]]$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),
                          id = Caribou_BM_SU_LI[[l]]$AnimalID, typeII = TRUE)
...
for(l in 1:length(Caribou_BM_RU_LI)){
  BM_RU_Traj[[l]] <-  as.ltraj(xy = Caribou_BM_RU_LI[[l]][,c("E", "N")], 
                          date =  as.POSIXct(Caribou_BM_RU_LI[[l]]$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),
                          id = Caribou_BM_RU_LI[[l]]$AnimalID, typeII = TRUE)
...
for(l in 1:length(Caribou_TR_WI_LI)){
  TR_WI_Traj[[l]] <-  as.ltraj(xy = Caribou_TR_WI_LI[[l]][,c("E", "N")], 
                          date =  as.POSIXct(Caribou_TR_WI_LI[[l]]$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),
                          id = Caribou_TR_WI_LI[[l]]$AnimalID, typeII = TRUE)
...
for(l in 1:length(Caribou_TR_CA_LI)){
  TR_CA_Traj[[l]] <-  as.ltraj(xy = Caribou_TR_CA_LI[[l]][,c("E", "N")], 
                          date =  as.POSIXct(Caribou_TR_CA_LI[[l]]$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),
...
for(l in 1:length(Caribou_TR_SU_LI)){
  TR_SU_Traj[[l]] <-  as.ltraj(xy = Caribou_TR_SU_LI[[l]][,c("E", "N")], 
                          date =  as.POSIXct(Caribou_TR_SU_LI[[l]]$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),
...
for(l in 1:length(Caribou_TR_RU_LI)){
  TR_RU_Traj[[l]] <-  as.ltraj(xy = Caribou_TR_RU_LI[[l]][,c("E", "N")], 
                          date =  as.POSIXct(Caribou_TR_RU_LI[[l]]$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),

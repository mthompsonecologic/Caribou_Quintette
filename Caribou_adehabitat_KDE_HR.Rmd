...
h2{ Glencore: Quintette Caribou, Bullmoose and Trend Roman : R Markdown code}

##################################################################
## Qunitette caribou home range analysis:
## Prepared for: Glencore and Scott MckNay - Wildlife Infometrics
...
if(!require(BBMM)){install.packages("BBMM")}
if(!require(lunar)){install.packages("lunar")}
...
if(!require(furniture)){install.packages("furniture")}
...
h1{PART 1 - AdehabitatHR Script for KDE}
...
On Aug 11, 2022 - Scott McNay sent a new excel file ("qttelemfinal6.csv") with cleaner telemetry data. This is the code for processing this into the existing script.
...
h2{Import the shapefiles of the study areas}
## (SA) : BM = Bullmoose, TR = Trend Roman, "All" is BM+TR
...
## Animals in study areas - these are animals in the clipped study area
##
...
The following does not clip animals to the study area. Skip this chunk to keep the data trimmed to the perimeter for mapping purposes only. This chunk of code also determines the maximum extent of the points that are used later for trimming the raster images for movement distance analysis.
...
After coding this in, I learned that lag and step can also be calculated using the move package: Section 5 Extracting temporal and spatial information of tracks
https://cran.r-project.org/web/packages/move/vignettes/move.html

However, these loops produce the same result. They calculate the lag time in minutes between sample intervals
of the same animal. It puts each animal into a list with the lag time calculated. The first lag entry is marked "NA".
...
It takes >30 minutes to compute this code chunk. This can be skipped by loading the files in lines 352 and 364 below, but all chunks above and those below prior to the UD calculations need to be re-run to enter into the UD calculations.
...
names(Caribou_BM)[names(Caribou_BM) == "season"] <- "Season2" ## To check Scott's seasons to the script I wrote.
...
names(Caribou_TR)[names(Caribou_TR) == "season"] <- "Season2" ## To check Scott's seasons to the script I wrote.


## Reduced to:
print(names(Caribou_TR))
##[1] "AnimalID" "E" "N" "KDESEASON" "timestamp" "lag" "step"

```

The animals are reset in this chunk after things have been vetted above. Depending on the list or threshold settings above, some animals can be removed. Hence, the reset needs to be performed.


```{r Table Animals, echo=FALSE, results = "hide"}
...
## The study years:
Caribou_BM_Yrs <- unique(format(as.Date(Caribou_BM[,5], format="%Y-%m-%d %H:%M:%S"),"%Y"))
Caribou_TR_Yrs <- unique(format(as.Date(Caribou_TR[,5], format="%Y-%m-%d %H:%M:%S"),"%Y"))

#Bullmoose
## Creating the aggregated data & animals

Animals_Agg <- c(Animals_BM,Animals_TR)
length(Animals_Agg) ## 49 animals
...
Caribou_BM_Agg_Yrs <- sort(unique(format(as.Date(Caribou_BM[,5], format="%Y-%m-%d %H:%M:%S"),"%Y")))
...
The data also has to deal with Nov 1-Dec 31 in year-1.This will be handled by putting the Nov 1-Dec 31 section into the year of Jan-1 to end Apr. For example, Nov 1, 2007 until Apr 30 2008 is Winter 2008. The function assign_seasonYR is built to address this.
...
for(i in 1:length(years)){
   if (!is.wholenumber(as.numeric(years[i]) / 4))
       {
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-11-01") & Caribou_BM$date <= paste0(years[i],"-12-31"),11] = "EW"
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-01-01") & Caribou_BM$date <= paste0(years[i],"-02-28"),11] = "MW"
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-03-01") & Caribou_BM$date <= paste0(years[i],"-04-30"),11] = "LW"
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-05-01") & Caribou_BM$date <= paste0(years[i],"-06-30"),11] = "CA"
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-07-01") & Caribou_BM$date <= paste0(years[i],"-08-31"),11] = "SU"
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-09-01") & Caribou_BM$date <= paste0(years[i],"-10-31"),11] = "RU"
   }
  else if(!is.wholenumber(as.numeric(years[i]) / 100)){
      Caribou_BM[Caribou_BM$date >= paste0(years[i],"-11-01") & Caribou_BM$date <= paste0(years[i],"-12-31"),11] = "EW"
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-01-01") & Caribou_BM$date <= paste0(years[i],"-02-29"),11] = "MW"
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-03-01") & Caribou_BM$date <= paste0(years[i],"-04-30"),11] = "LW"
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-05-01") & Caribou_BM$date <= paste0(years[i],"-06-30"),11] = "CA"
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-07-01") & Caribou_BM$date <= paste0(years[i],"-08-31"),11] = "SU"
    Caribou_BM[Caribou_BM$date >= paste0(years[i],"-09-01") & Caribou_BM$date <= paste0(years[i],"-10-31"),11] = "RU"
...
assign_seasonYR <- function(d) {
  ifelse(format(as.Date(d, format = "%Y-%m-%d", tz = "GMT"),"%m") == "11" & format(as.Date(d, format =  "%Y-%m-%d %H:%M:%S", tz = "GMT"),"%d") >= "1", as.numeric(format(as.Date(d, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"),"%Y"))+1,
  ifelse(format(as.Date(d, format =  "%Y-%m-%d", tz = "GMT"),"%m") == "12" & format(as.Date(d, format =  "%Y-%m-%d", tz = "GMT"),"%d") <= "31", as.numeric(format(as.Date(d, format = "%Y-%m-%d", tz = "GMT"),"%Y")), as.numeric(format(as.Date(d, format = "%Y-%m-%d", tz = "GMT"),"%Y"))))
}
...
for(i in 1:length(years)){
   if (!is.wholenumber(as.numeric(years[i]) / 4))
    ...
    Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-01-01") & Caribou_BM_Agg$date <= paste0(years[i],"-02-28"),11] = "MW"
    Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-03-01") & Caribou_BM_Agg$date <= paste0(years[i],"-04-30"),11] = "LW"
    Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-05-01") & Caribou_BM_Agg$date <= paste0(years[i],"-06-30"),11] = "CA"
    Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-07-01") & Caribou_BM_Agg$date <= paste0(years[i],"-08-31"),11] = "SU"
    Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-09-01") & Caribou_BM_Agg$date <= paste0(years[i],"-10-31"),11] = "RU"
   }
  else if(!is.wholenumber(as.numeric(years[i]) / 100)){
      Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-11-01") & Caribou_BM_Agg$date <= paste0(years[i],"-12-31"),11] = "EW"
    Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-01-01") & Caribou_BM_Agg$date <= paste0(years[i],"-02-29"),11] = "MW"
    Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-03-01") & Caribou_BM_Agg$date <= paste0(years[i],"-04-30"),11] = "LW"
    Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-05-01") & Caribou_BM_Agg$date <= paste0(years[i],"-06-30"),11] = "CA"
    Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-07-01") & Caribou_BM_Agg$date <= paste0(years[i],"-08-31"),11] = "SU"
    Caribou_BM_Agg[Caribou_BM_Agg$date >= paste0(years[i],"-09-01") & Caribou_BM_Agg$date <= paste0(years[i],"-10-31"),11] = "RU"
...
assign_seasonYR <- function(d) {
  ifelse(format(as.Date(d, format = "%Y-%m-%d", tz = "GMT"),"%m") == "11" & format(as.Date(d, format =  "%Y-%m-%d", tz = "GMT"),"%d") >= "1", as.numeric(format(as.Date(d, format = "%Y-%m-%d", tz = "GMT"),"%Y"))+1,
  ifelse(format(as.Date(d, format =  "%Y-%m-%d", tz = "GMT"),"%m") == "12" & format(as.Date(d, format =  "%Y-%m-%d", tz = "GMT"),"%d") <= "31", as.numeric(format(as.Date(d, format = "%Y-%m-%d", tz = "GMT"),"%Y")), as.numeric(format(as.Date(d, format = "%Y-%m-%d", tz = "GMT"),"%Y"))))
}
...
Caribou_BM_Agg[which(Caribou_BM_Agg$season == "EW" | Caribou_BM_Agg$season == "MW" | Caribou_BM_Agg$season == "LW"),11] <- "WI"
...
BM - The Four Season Model with winter as one.
Create	a	list	of	SpatialPointsDataFrames: By Season x Year
...
Caribou_TR_Agg_Yrs <- sort(unique(format(as.Date(Caribou_TR[,5], format="%Y-%m-%d %H:%M:%S"),"%Y")))
...
for(i in 1:length(years)){
   if (!is.wholenumber(as.numeric(years[i]) / 4))
       {
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-11-01") & Caribou_TR$date <= paste0(years[i],"-12-31"),11] = "EW"
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-01-01") & Caribou_TR$date <= paste0(years[i],"-02-28"),11] = "MW"
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-03-01") & Caribou_TR$date <= paste0(years[i],"-04-30"),11] = "LW"
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-05-01") & Caribou_TR$date <= paste0(years[i],"-06-30"),11] = "CA"
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-07-01") & Caribou_TR$date <= paste0(years[i],"-08-31"),11] = "SU"
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-09-01") & Caribou_TR$date <= paste0(years[i],"-10-31"),11] = "RU"
   }
  else if(!is.wholenumber(as.numeric(years[i]) / 100)){
      Caribou_TR[Caribou_TR$date >= paste0(years[i],"-11-01") & Caribou_TR$date <= paste0(years[i],"-12-31"),11] = "EW"
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-01-01") & Caribou_TR$date <= paste0(years[i],"-02-29"),11] = "MW"
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-03-01") & Caribou_TR$date <= paste0(years[i],"-04-30"),11] = "LW"
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-05-01") & Caribou_TR$date <= paste0(years[i],"-06-30"),11] = "CA"
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-07-01") & Caribou_TR$date <= paste0(years[i],"-08-31"),11] = "SU"
    Caribou_TR[Caribou_TR$date >= paste0(years[i],"-09-01") & Caribou_TR$date <= paste0(years[i],"-10-31"),11] = "RU"
...
for(i in 1:length(years)){
   if (!is.wholenumber(as.numeric(years[i]) / 4))
       {
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-11-01") & Caribou_TR_Agg$date <= paste0(years[i],"-12-31"),11] = "EW"
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-01-01") & Caribou_TR_Agg$date <= paste0(years[i],"-02-28"),11] = "MW"
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-03-01") & Caribou_TR_Agg$date <= paste0(years[i],"-04-30"),11] = "LW"
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-05-01") & Caribou_TR_Agg$date <= paste0(years[i],"-06-30"),11] = "CA"
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-07-01") & Caribou_TR_Agg$date <= paste0(years[i],"-08-31"),11] = "SU"
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-09-01") & Caribou_TR_Agg$date <= paste0(years[i],"-10-31"),11] = "RU"
   }
  else if(!is.wholenumber(as.numeric(years[i]) / 100)){
      Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-11-01") & Caribou_TR_Agg$date <= paste0(years[i],"-12-31"),11] = "EW"
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-01-01") & Caribou_TR_Agg$date <= paste0(years[i],"-02-29"),11] = "MW"
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-03-01") & Caribou_TR_Agg$date <= paste0(years[i],"-04-30"),11] = "LW"
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-05-01") & Caribou_TR_Agg$date <= paste0(years[i],"-06-30"),11] = "CA"
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-07-01") & Caribou_TR_Agg$date <= paste0(years[i],"-08-31"),11] = "SU"
    Caribou_TR_Agg[Caribou_TR_Agg$date >= paste0(years[i],"-09-01") & Caribou_TR_Agg$date <= paste0(years[i],"-10-31"),11] = "RU"
...
Caribou_TR_Agg[which(Caribou_TR_Agg$season == "EW" | Caribou_TR_Agg$season == "MW" | Caribou_TR_Agg$season == "LW"),11] <- "WI"
...
for(i in 1:length(Caribou_BM_Agg_Yrs_SPDF)){
  if(!is.null(Caribou_BM_Agg_Yrs_SPDF[[i]])){
...
p + geom_line(data = Caribou_BM_DF, aes(x = Date, y = Animals_n * 100),linetype = "dotted", color = 'black') +
    ...
    panel.grid.minor.x = element_blank()) + geom_rect(aes(xmin = as.Date("2005-01-01"),xmax = as.Date("2013-01-01"),ymin = 0, ymax = Inf), fill="grey", alpha = .2) +
...
Plotting the four season model
...
```
h2{Adehabitat Kernel Utilization Density Models}

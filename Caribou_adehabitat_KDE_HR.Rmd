...
Package and library installs were broken down for use in the Acenet high-performance computing environment. Change eval condition as needed.

```{r Acenet_package, eval=FALSE}
# For Acenet:
install.packages(c("sp", "terra","rgdal","lubridate","maptools","adehabitatHR","raster","lunar","oce","summarytools","move","#ggplot2","sf","rgeos","stringr","dismo","ggmap","mapproj","maptools","here","rcompanion","gr#idExtra","RColorBrewer","furniture","gtable","foreach","doParallel"))
...
# Winterpar <- c(seasonal[[1]],seasonal[[5]])
# Calvingpar <- c(seasonal[[2]],seasonal[[6]])
# Summerpar <- c(seasonal[[3]],seasonal[[7]])
# Ruttingpar <- c(seasonal[[4]],seasonal[[8]])

# seasonFun(Winterpar)

# Create a new list to store the processed data frames
season_list <- vector("list", length(seasonal))

#Separate out the seasons foreach parallel
Winterpar <- c(seasonal[[1]],seasonal[[5]])
Winterpar_list <- vector("list", length(Winterpar))

Calvingpar <- c(seasonal[[2]],seasonal[[6]])
Calvingpar_list <- vector("list", length(Calvingpar))

Summerpar <- c(seasonal[[3]],seasonal[[7]])
Summerpar_list <- vector("list", length(Summerpar))

Ruttingpar <- c(seasonal[[4]],seasonal[[8]])
Ruttingpar_list <- vector("list", length(Ruttingpar))
...
# Bring all seasons together here:
#UD_list <- c(Winterpar_list,Calvingpar_list,Summerpar_list,Ruttingpar_list)

ver <- list()

ver <- foreach(i = 2:length(Winterpar_list),
               .combine = 'c',
               .packages = "adehabitatHR")  %dopar% {
                    getverticeshr(Winterpar_list[[i]],
                                  percent = per)
                }
...
# Calculate the areas and write the shapefiles:
UD_area <- data.frame()
...
TabUD_area[nrow(UD_area),c(1:5)] <- rbind(data.frame(SA =substr(names(seasonal[[s]][i]),1,2), season = substr(names(seasonal[[s]][i]),13,14), year = substr(names(seasonal[[s]][i]),8,11), area = UD_area[[tally]], nb.reloc = length(seasonal[[s]][[i]])))
